# gprob
gprob is a probabilistic programming language for Gaussian random variables with exact conditioning, implemented as a python package. With gprob, arrays of Gaussian random variables can be manipulated in the same ways as numerical arrays are manipulated using numpy.

To give a flavor of it, operations on scalar random variables can look as follows
```python
>>> import gprob as gp
>>> x = gp.normal()
>>> y = gp.normal()
...
As another example, we can construct the distribution of trajectories x(t) of a Brownian particle observed with x=0 at t=0 and x=1 at t=1/2:
```python
>>> nstep = 5 * 10**3
>>> dx = gp.normal(0, 1/nstep, size=(nstep,))
>>> x = gp.cumsum(dx, 0)  # unconditional particle positions
>>> xc = x | {x[nstep//2]: 1}  # conditional positions
...
5. [Information theory](examples/5-information-theory.ipynb)
6. [Stochastic differential equations](examples/6-sde.ipynb)

and going roughly in this order.
...
## Acknowledgements
gprob was inspired by (but works differently from) [GaussianInfer](https://github.com/damast93/GaussianInfer). See the paper
...
The implementation of gprob uses the subscript parser from [opt-einsum](https://github.com/dgasmith/opt_einsum). Some linearization tricks and choices of tooling follow [autograd](https://github.com/HIPS/autograd).
